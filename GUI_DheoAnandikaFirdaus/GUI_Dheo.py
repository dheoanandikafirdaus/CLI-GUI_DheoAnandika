...
# fungsi untuk menampilkan plot 3D (sembarang)
def plot_sinus_3d():
    try:
        ...
        R = np.sqrt(X**2 + Y)        # hitung jarak dari pusat (radius)
        Z = amp*np.sin(freq/R)      # rumus fungsi sinus bebas dehh
        ...
        ax.set_title("Plot 3D Fungsi Sin Sembarang")   # judul grafik
...
# buat jendela utama
root = tk.Tk()
root.title("Plot 3D Fungsi Sin Sembarang")

tk.Label(root, text="Masukkan elemen fungsi f(x) = A*sin(f/√(x² + y))").pack(pady=10)
